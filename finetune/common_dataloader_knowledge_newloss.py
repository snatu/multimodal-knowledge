...
#         option_i = tf.concat([option_i, knowledge], 0)
        ...
        # Add the knowledge
        knowledge_seqs.append(pad_to_fixed_size(knowledge, pad_value=0, output_shape=[config['lang_seq_len']]))

    features['textonly_seqs'] = tf.stack(textonly_seqs)
    features['audio_seqs'] = tf.stack(audio_seqs)
    features['labels'] = features.pop('qa_label')
    features['knowledge'] = tf.reshape(tf.stack(knowledge_seqs, 0), [1, config['num_answers'], config['lang_seq_len']])
