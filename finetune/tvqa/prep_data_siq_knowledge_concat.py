...
split_fn_knowledge = {
    'train': 'siq_train_knowledge.jsonl',
    'val': 'siq_val_knowledge.jsonl',
    #'test': 'tvqa_test_public.jsonl',
}[args.split]
split_fn_knowledge = os.path.join(args.data_dir, 'siq_qa_release', split_fn_knowledge)
...
data_knowledge = []
with open(split_fn_knowledge, 'r') as f:
    for idx, l in enumerate(f):
        if idx % args.num_folds != args.fold:
            continue
        item = json.loads(l)
...
def parse_item(item, knowledge):
    ...
    knowledge_num = 0
    knowledges = []
    while f"a{knowledge_num}" in knowledge:
        knowledges.append(knowledge.pop(f"a{knowledge_num}"))
        knowledge_num += 1
    correct_knowledge = knowledges[knowledge.get('answer_idx')]
    qa_item = {'qa_query': item.pop('q'), 'qa_choices': answer_choices,
               'qa_label': item.get('answer_idx', 0), 'knowledge': correct_knowledge,
...
with GCSTFRecordWriter(out_fn, auto_close=False) as tfrecord_writer:
    for i, item in enumerate(data):
        qa_item, frames, specs, subs = parse_item(item, data_knowledge[i])
