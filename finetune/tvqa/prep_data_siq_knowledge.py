...
out_fn = os.path.join(args.base_fn, 'siq_knowledge', '{}{:03d}of{:03d}.tfrecord'.format(args.split, args.fold, args.num_folds))
...
split_fn_knowledge = os.path.join(args.data_dir, 'siq_qa_release', 'siq_train_knowledge.jsonl')

data = []
with open(split_fn, 'r') as f:
    with open(split_fn_knowledge, 'r') as k:
        for idx, (l, m) in enumerate(zip(f, k)):
