...
out_fn = os.path.join(args.base_fn, 'siq_viscom', '{}{:03d}of{:03d}.tfrecord'.format(args.split, args.fold, args.num_folds))
...
with open('/data/train_results.txt', 'r') as f:
    for line in f:
        ...
        if knowledge[1][:2] == " ,":
            knowledge[1] = ""
        vid_knowledge[knowledge[0]] = knowledge[1]
...
def parse_item(item):
    ...
    for trow in times_used0:
        ...
        if os.path.exists(fn):
            ...
            frames.append(image)
            vid_name = item['vid_name'] + "_trimmed-out_" + f'{t_mid_3ps_idx:03d}.jpg'
            if vid_name in vid_knowledge:
                trow['knowledge'] = vid_knowledge[vid_name]
            else:
                trow['knowledge'] = ""
    ...
    for ts in times_used:
        ...
        ts['knowledge'] = unidecode(ftfy.ftfy(ts['knowledge'])).replace('\n', ' ')
