...
class MerlotReserveTVQA(MerlotReserve):
    ...
    def __call__(self, batch):
        ...
        #########################
        if 'knowledge' in batch:
            batch_size, _, num_ans_per, token_length = batch['knowledge'].shape
            knowledge_toks = batch['knowledge'].reshape(batch_size *num_ans_per, token_length)
            knowledgeonly_inputs = self.prepare_multimodal_inputs(
                tokens=knowledge_toks,
                token_segment_idx=jnp.zeros([batch_size* num_ans_per, token_length], dtype=jnp.int32),
                vision_input=imgs_enc,
            )
        ...
        for k in ['x', 'rotary_coords', 'attention_mask']:
            ...
            if 'knowledge' in batch:
                knowledgeonly_inputs[k] = jnp.concatenate([knowledgeonly_inputs[k][:, :joint_seq_len],
                                      knowledgeonly_inputs[k][:, start_imgs:]], 1)
        ...
        if 'knowledge' in batch:
            knowledgeonly_inputs['attention_mask'] = jnp.concatenate([knowledgeonly_inputs['attention_mask'][:, :, :joint_seq_len],
                                                     knowledgeonly_inputs['attention_mask'][:, :, start_imgs:]], 2)
        ...
        if 'knowledge' in batch:
            x = jnp.concatenate([audio_inputs['x'], textonly_inputs['x'], knowledgeonly_inputs['x']], 0)
            coords = jnp.concatenate([audio_inputs['rotary_coords'], textonly_inputs['rotary_coords'], knowledgeonly_inputs['rotary_coords']], 0)
            attnmask = jnp.concatenate([audio_inputs['attention_mask'], textonly_inputs['attention_mask'], knowledgeonly_inputs['attention_mask']], 0)

            joint_enc = self.joint_transformer(x, rotary_coords=coords, attention_mask=attnmask)['seq']
            joint_enc = joint_enc[:, :joint_seq_len].reshape(batch_size * 3 * num_ans_per, joint_seq_len, self.hidden_size)
            ...
            pooled_h = joint_enc[jnp.arange(batch_size * 3 * num_ans_per), pool_idx]
...
def train_loss_fn(state, params, batch):
    ...
    return loss, {'train_audio_acc': is_right_audio, 'train_text_acc': is_right_text, 'train_knowledge_acc': is_right_knowledge,
                  'train_audio_loss': loss_audio, 'train_text_loss': loss_text, 'train_knowledge_loss': loss_knowledge}
